...
> > 문제 정의 및 해결과제
>
> - 유동인구에 대한 데이터의 분석을 통해 저희는 사회적 거리두기 해제에 따라 유동인구가 예년과 비슷한 수준까지 회복되었다는 것을 알 수 있었습니다. 이에따라 길거리에 버려지는 쓰레기가 급증하고 있다는 정보 또한 기사를 통해 접하게 되었습니다. 따라서 최근 거리두기 해제에 따라 인구가 물리면서 거리에 버려지는 쓰레기로 인한 환경 문제가 심각해지고 있다는 결론에 도달하게 되었고 쓰레기를 무단투기하는 행위 자체는 차치하더라도 버려진 쓰레기만은 해결하는 쪽으로 웹서비스의 방향을 정했습니다. 좋은 방안이 있을까 고민하던 차에 플로깅이라는 아이디어가 떠올랐고 조깅이라는 것보다 더 대중적인 산책을 접목시키기로 논의하였습니다.  
>   <br>
>   > 데이터 분석
> - 위의 두 문제를 해결하기 위해 산책로 데이터와 쓰레기통 데이터를 사용했습니다.
>   - 산책로 데이터에서 서비스를 만드는데 불필요한 열들을 삭제해주었고 남은 열들의 관계가 수평적이지 않고 포함관계인 것들이 많아 중복된 데이터가 있었기 때문에 데이터프레임을 수정해서 사용하기 용이하도록 만들었습니다.
>   - 흔히 사용하는 위도, 경도 좌표계가 아닌 다른 좌표계로 표시되어 있어 pyproj 라이브러리를 활용하여 좌표계를 변환해 주었고 다른 열들도 자료형의 수정이 필요한 경우 수정해주었습니다.
>   - 추가적인 작업을 통해 지도에 좌표를 표시했을 때 실제 산책코스로서 사용할 수 있도록 좌표를 추가하는 작업을 진행했습니다.
>
> > 쓰레기통 데이터는 서울시에 설치된 쓰레기통의 종류와 주소로 이루어진 데이터로 앞서 말한 산책로 데이터와 결합하여 경로의 도착지엔 사용자들이 활동을 하면서 수거한 쓰레기를 처리할 수 있도록 쓰레기통이 위치하는 저희 서비스만의 데이터를 만들었습니다.
>
> <br>
...
- #### 프로젝트만의 차별점, 기대 효과
  ...
  > > 차별점
  >
  > - 플로깅이라는 활동은 흔하게 접할 수는 없었을지라도 개인적인 활동이나 기업 행사 등에서 종종 있어 왔습니다. 하지만 기사나 블로그에서 찾아 볼 수 있는 플로깅의 문제점은 바로 쓰레기의 처리입니다. 쓰레기를 마땅히 버릴 곳이 없어 집으로 들고 올 수 밖에 없다는 것입니다([관련기사](https://imnews.imbc.com/replay/2022/nwtoday/article/6390901_35752.html))  
  >   분명 선한 마음에서 활동을 하였는데 결과적으로는 본인에게 손해가 될 수 있다면 활동을 하지 않게되는 일도 늘어날 것입니다. 저희는 이 부분에 대하여 차별점을 두고 있습니다. 앞서 말한 부분을 가장 큰 문제점이라고 인지하고 있고 이에 따라 도착지를 쓰레기통으로 설정함으로써 문제를 해결했습니다.  
  >    <br>
  >
  > > 기대효과
  >
  > - 기대 효과 또한 상술한 차별점과 연관이 있습니다. 좋은 활동에서 걸림돌이 되는 문제를 해결함으로써 진입장벽을 낮춰 주었고, 또한 혼자하는 운동보다는 여럿이 하는 운동이 동기부여 측면에서 더 좋을 것이라 판단, 함께 실천하는 마을 커뮤니티를 형성할 수 있도록 모임을 형성했습니다. 따라서 커뮤니티라는 지속할 동기 또한 형성하여 이 둘의 상승 효과로 건강과 환경 모두 챙길 수 있을 것으로 기대됩니다.  
  >   <br>
